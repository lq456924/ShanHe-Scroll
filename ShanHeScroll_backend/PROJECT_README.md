# ShanHeScroll 山河卷 - 旅游社交平台

## 项目简介

ShanHeScroll 是一个结合**旅游推荐**与**社交分享**的 Web 应用后端项目。用户可以在平台上浏览全国各地区的旅游景点推荐，创建个人旅拍相册，并通过"漂流瓶"功能与其他旅行爱好者互动。
...
├── ShanHeScrollApplication.java                  ← 启动类
├── common/
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/file/upload` | 单文件上传（≤20MB，仅图片） |
...
```
发送漂流瓶 → 选择是否匿名 → 状态设为"待审核" → 审核员审核通过后进入漂流派送池
